...
public class DuolingoResponse : MonoBehaviour
{
    ...
    void Update()
    {
        if(call) showExpresion(); //Si se llamo a la funcion desde otro script se habilita
        else textSpace.SetActive(false); //Si no no
    }
    ...
    void incorrect()
    {
        response.text = "You are Stupid";
    }

    public void showExpresion()
    {
        if(isCorrect) correct(); //Se edita el texto dependiendo de la respuesta
        else incorrect();
        textSpace.SetActive(true); //Y se activa el texto
    }
}
